...
import master.sedemo.tasks.Currency;
import master.sedemo.tasks.*;
import master.sedemo.system.TablePrinter;
 ...
 * {@link master.sedemo.tasks}.
 * <code>
 * <a href="{@docRoot}/index.html">{@value master.sedemo.application.package_info#RootName}</a>.
 * </code>
 * 
...
public class Application_E1_complete {
	//
	 ...
	 */
	private Application_E1_complete() {
		System.out.println(master.sedemo.application.package_info.RootName + ": " + this.getClass().getSimpleName());
		//
	...
	public static void main(String[] args) {
		var appInstance = new Application_E1_complete();
		appInstance.run();
	}
...
//		.line();
	...
	private void moreOrders() {
		...
		Article karte = new Article("Fahrradkarte", 695).setId("SKU-583978")
				.setTax(TAX.GER_VAT_REDUCED);	// reduced tax rate for books
		//
		this.articles.addAll(Arrays.asList(pfanne, helm, karte));
	...
	public TablePrinter printOrder(final TablePrinter orderTable, final Order order) {
		...
		if(orderTable != null && order != null) {
			...
			for(int i = order.itemsCount(); --i >= 0; ) {
				...
				totalVAT += vat;
				//
				String unitPriceStr = fmtPrice(unitPrice);
				...
				//
				String vatStr = fmtPrice(vat);
				String vatR = item.getArticle().getTax()==TAX.GER_VAT_REDUCED? "*" : "";
				String itemPriceStr = fmtPrice(itemPrice, 1);
				//
				if(i > 0) {
					orderTable.row("", lineItem, vatStr, vatR, itemPriceStr);
				} else {
					String totalPriceStr = fmtPrice(totalPrice, 1);
					String totalVATStr = fmtPrice(totalVAT, 1);
...
//				s -> s.filter(o -> o.getId().equals("8592356245") || o.getId().equals("3563561357")),
				order -> {
					long[] oval = calculateValueAndTax(order);
